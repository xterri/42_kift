...
/*   Updated: 2017/09/22 03:45:49 by thuynh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*when_alarm_is_set(int time, int time_measurement)
{
	...
	if (time == 1)
		...
		plural = 1;
	phrase1 = ft_strdup("alarm will be set to go off in ");
	snprintf(num, 30, "%d", time);
	phrase2 = ft_strdup(num);
...
char		*alarm_response(char *str, int negation)
{
	...
	if (negation)
		return (ft_strdup("ok I won't set an alarm."));
	time = get_first_num(str);
	time_measurement = find_time_phrase(str);
	if (!time)
		return (ft_strdup("I don't know when to set the alarm."));
	if (!time_measurement)
	{
		if (time < 5 || time % 5 != 0)
